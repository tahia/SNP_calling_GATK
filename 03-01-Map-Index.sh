...
echo "java -jar -Xmx24G $TACC_PICARD_DIRCreateSequenceDictionary.jar R=$refDir/$ref O=$refDir/${ref%.fa}.dict " >>\
    index.param
echo "samtools faidx $refDir/$ref" >>index.param

sbatch -J index -N 3 -n 3 -p development --ntasks-per-node=1 -t 02:00:00 slurm.sh index.param
