...
############################################# Step 14-02: MERGE gVCF ####################################################


# HaplotypeCaller(HC) is better in terms of miscall of heterozygosity compare to UnifiedGenotyper(UG)
# This is script is for gVCF produced by HC


if [ -e mergevcfg.param ]; then rm mergevcfg.param; fi

PREF="--variant "
IN=""
for f in `ls $outDir/FinalVCF/*.filter.g.vcf`
do
    IN+="${PREF}${f} "
done
    OFIL="${outDir}/FinalVCF/AllRIL_merge_vcf.vcf"

echo "java -jar -Xmx24G \$TACC_GATK_DIR/GenomeAnalysisTK.jar -T GenotypeGVCFs -R $refDir/$ref -o $OFIL $IN --includeNonVariantSites" >mergevcfg.param
